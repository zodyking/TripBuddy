...
/**
 * Plain-text block for Dispatch Instructions when FedEx clears instruction fields after DSPCH.
 * Lists dolly + each trailer number, seal (when on summary), size, load, door status.
 * @param {unknown} body
 * @returns {string}
 */
export function formatTripEquipmentDispatchAppendix(body) {
  try {
    if (body == null || typeof body !== 'object' || Array.isArray(body)) return ''
    const o = /** @type {Record<string, unknown>} */ (body)
    /** @type {string[]} */
    ...
    if (dolly.show) {
      const parts = dolly.rows
        .filter((r) => r.value !== '—' && String(r.value).trim() !== '')
        .map((r) => `${r.label}: ${r.value}`)
      if (parts.length) lines.push(`Dolly — ${parts.join(' · ')}`)
    }

    const arr = o.trailers
    if (!Array.isArray(arr) || arr.length === 0) {
      if (!lines.length) return ''
      return ['Equipment (from trip)', ...lines.map((l) => `• ${l}`)].join('\n')
    }
    ...
    for (const { tr, i } of indexed) {
      const order = tr.trlrOrder != null ? String(tr.trlrOrder) : String(i + 1)
      const meta = parseTrailerMeta(tr)
      const seal =
        meta.sealNumber && meta.sealNumber !== '—' && String(meta.sealNumber).trim() !== ''
          ? String(meta.sealNumber).trim()
          : ''

      /** @type {string[]} */
      const bits = []
      if (meta.trlrNbr) bits.push(`#${meta.trlrNbr}`)
      else bits.push('(number pending)')
      if (meta.size) bits.push(meta.size)
      if (seal) bits.push(`Seal ${seal}`)
      if (meta.loadType && meta.loadType !== '—') bits.push(meta.loadType)
      if (meta.statusLabel && meta.statusLabel !== '—') bits.push(meta.statusLabel)

      lines.push(`Trailer ${order} — ${bits.join(' · ')}`)
    }

    if (!lines.length) return ''
    return ['Equipment (from trip)', ...lines.map((l) => `• ${l}`)].join('\n')
  } catch (e) {
    console.error('[formatTripEquipmentDispatchAppendix]', e)
    return ''
  }
}
